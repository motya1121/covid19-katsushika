...
# 手動で日時を設定
# updated_datetime = datetime.datetime.strptime('2020-04-18', '%Y-%m-%d')
# 現在の時刻を採用
        ...
        if i == 0:
            ...
            temp_data = {
                ...
                'status': {
                    ...
                    '宿泊療養中': True if row[8] == '1' else False,
                    '自宅療養中': True if row[9] == '1' else False,
                    '死亡': True if row[10] == '1' else False,
                    '退院': True if row[11] == '1' else False
                },
                'symptoms': {
                    '発熱': True if row[12] == '1' else False,
                    '咳': True if row[13] == '1' else False,
                    '咽頭痛': True if row[14] == '1' else False,
                    '味覚障害': True if row[15] == '1' else False,
                    '嗅覚障害': True if row[16] == '1' else False,
                    '倦怠感': True if row[17] == '1' else False,
                    '下痢': True if row[18] == '1' else False,
                    '頭痛': True if row[19] == '1' else False,
                    '鼻汁': True if row[20] == '1' else False,
                    '関節痛': True if row[21] == '1' else False,
                    '鼻閉': True if row[22] == '1' else False,
                    '痰': True if row[23] == '1' else False
                }
            }
...
for row_data in row_datas:
    ...
    if row_data['status']['入院中'] is True:
        status_sumary['入院中'] += 1
    if row_data['status']['宿泊療養中'] is True:
